...
class AirBnbComponent extends Component
{
    ...
    public function mount()
    {
        ...
        if (session()->has('air_bnb_result')) {
            if (session()->has('air_bnb_result_expires_at') && Carbon::now()->gt(session()->get('air_bnb_result_expires_at'))) {
                session()->forget('air_bnb_result');
                session()->forget('air_bnb_result_expires_at');
                $this->result = [];
            } else {
                $this->result = session()->get('air_bnb_result');
            }
        }
        ...
        $this->air_bnb_used =   $activities->air_bnb_used;
    }
    ...
    public function searchData()
    {
        ...
        $response = Http::withHeaders([
            ...
            ])->get('https://wextractor.com/api/v1/reviews/airbnb', [
            ...
            if (isset($response->json()['detail'])) {
                $this->result = [];
                session()->flash('error', $response->json()['detail'] . ': Check air_bnb API key');
                return;
            } else {
            session()->put('air_bnb_result', $response->json()['reviews']);
            session()->put('air_bnb_result_expires_at', $expirationTime);
            return  $this->result = session()->get('air_bnb_result');
        }
    }
    ...
    {
        return view('livewire.air-bnb-component');
    }
}
