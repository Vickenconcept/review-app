...
class G2Component extends Component
{
    use WithPagination;
    ...
    public function mount()
    {
        ...
        if (session()->has('g2_result')) {
            if (session()->has('g2_result_expires_at') && Carbon::now()->gt(session()->get('g2_result_expires_at'))) {
                session()->forget('g2_result');
                session()->forget('g2_result_expires_at');
                $this->result = [];
            } else {
                $this->result = session()->get('g2_result');
            }
        }
        ...
        $this->g2_used =   $activities->g2_used;
    }
    ...
    public function searchData()
    {
        ...
        $response = Http::withHeaders([
            ...
            ])->get('https://wextractor.com/api/v1/reviews/g2', [
            ...
            if (isset($response->json()['detail'])) {
                $this->result = [];
                session()->flash('error', $response->json()['detail'] . ': Check g2 API key');
                return;
            } else {
            session()->put('g2_result', $response->json()['reviews']);
            session()->put('g2_result_expires_at', $expirationTime);
            return  $this->result = session()->get('g2_result');
        }
    }
    ...
    {
        return view('livewire.g2-component');
    }
}
